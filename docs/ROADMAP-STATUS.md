...
| F4 | Salvage/recuperação de banco corrompido (camadas L0-L5 + extrator de texto) | Feito — L0 cópia forense (uSafeCopy) + L1 validar/reparar gfix + L3 backup do que abre + orquestração (uSalvagePlan/uMotorSalvage) **entregues e verdes via FPC** (TestSalvagePlan: 82 checks, inclui E2E com fake gfix/gbak FPC); **L2 datapump tabela a tabela com driver REAL (uDriverFBClient)** ligado no fluxo automático — cada tabela exportada para CSV pulando as corrompidas (validado: 283/283 tabelas, 23.880 registros); **L2b reconstrução do banco a partir dos CSVs** (DDL isql -extract + INSERTs); L4 extrator de texto **ok** (uExtratorTexto, TestExtratorTexto: 59 checks); F4-T3 fluxo na UI **entregue** (dcc32) |
| F5 | Exportacao (PLANO 4.3/4.6/6.2: FBK, SQL/DDL, CSV/TSV e relatorio; regras transversais de formatos/nomenclatura) | Parcial — base (uExportBase), FBK (uExportFBK: gbak -b via subprocesso), SQL/DDL (uExportSQL: isql -extract com **captura do stdout**, sem redirecionamento '>' de shell, charset de saida aplicado), relatorio legivel do DDL (uExportReport: `<base>_ddl.txt` com contagem aproximada sobre os bytes) e CSV/TSV (uExportCSV: RFC-4180, NULL como campo vazio, BLOB por politica omitir/hex/arquivo lateral) **entregues e verdes via FPC** (TestExportFbk 32 + TestExportSql 36 + TestExportCsv 50 + TestExportReport 20 = 138 checks, com E2E via fakes compilados pelo FPC e driver de leitura fake em memoria); **driver real de dados `uDriverFBClient` (fbclient.dll 32-bit) entregue e ligado no datapump L2/L2b** (TestDriverFB validado com Firebird 2.5 embarcado); fluxo de exportacao CSV por tabela autonomo na UI **aguarda a UI** (dcc32) |
| F2-B, F6..F8 | Restante das engines, diagnostico na GUI, instalador etc. | Nao iniciadas |
| D7 (dcc32) | Validacao com Delphi 7 real instalado | **Feito (30103e9 + proximo)** — 18 programas de teste compilam e rodam com exit 0 no dcc32; correcoes de portabilidade D7 (aliases de array dinamico, bindings PWideChar(WideString(...)), leitura segura/limitada de registro em uFBAutoDetect). **GUI v1 funcional integrada** (uFrmMain): abre .fbk/.gbk/.fdb, auto-deteccao de bins (detectou InterBase 6.5 real do Delphi 7), restaura via gbak em worker thread com cancelar e resultado no memo; **recuperacao automatica completa ligada** (uMotorAutoRec: T1/T2/T4 p/ backup e L0-L4 + datapump L2/L2b p/ banco, relatorio em 5 secoes no painel e em `recuperacao\relatorio_recuperacao.txt`, botao Copiar relatorio, contador ao vivo, lembrete de sessao); threads com FreeOnTerminate (sem deadlock na conclusao); FBRecStudio.exe 0.3.0 compila e roda. Faltam: F6 (framework visual estilo macOS), exportacao CSV por tabela autonoma na GUI, F7 (corpora com Firebird real) e F8 (instalador/assinatura) |
